...
app.post('/registro/inscripcion/ucbonline', validSchemaUsuario, async(req, res) => {
    ...
    if (!existe.status) {
        ...
        if (existe.result.rows.length > 0) {
            ...
            await updateUserNeo(DOC_IDENTIDAD, lms_id_usuario);
...
const updateUserNeo = async(doc_identidad, lms_id_usuario) => {
    // acutualizar datos de usuario con el mun_sec_persona
    const sqluser = "SELECT NUM_SEC FROM USUARIOS_UCB_ONLINE WHERE DOC_IDENTIDAD = :DOC_IDENTIDAD";
    const dataUser = await db.query(sqluser, [doc_identidad], true);
    num_sec_persona = dataUser.status ? dataUser.result.rows[0].NUM_SEC : ''
