...
app.post('/registro', validSchemaMateria, async(req, res) => {
    ...
    const sqlmateria = `DECLARE
        ...
        SELECT COUNT(*) INTO data FROM WEB.MATERIAS_UCB_ONLINE WHERE LMS_ID_MATERIA = ${lms_id_materia};
        IF data = 0 THEN
        INSERT INTO WEB.MATERIAS_UCB_ONLINE (LMS_ID_MATERIA, NOMBRE, FECHA_INICIO, FECHA_FIN, CREDITOS, ORGANIZACION, NUM_SEC_SERVICIO, COSTO) VALUES (${lms_id_materia}, '${nombre.toUpperCase()}', NULL, NULL, ${creditos}, '${organizacion.toUpperCase()}', ${num_sec_servicio}, ${costo});
        COMMIT;
        END IF;
        END;`;
